...
def main() -> dict:
    ...
    analyzer.generate()
    # print(analyzer.options.topmodule)
    ...
    if options.nobind:
        ...
        for tk, tv in sorted(terms.items(), key=lambda x: str(x[0])):
            print(tv.tostr())
        print("Blocked...\n")
        #### Tree is generated from here ####
        binddict = analyzer.getBinddict()
        print('Bind:')
        ## Added by vnay01
        if not os.path.exists('data_flow'):
            os.mkdir('data_flow')
        output_file=os.path.abspath('data_flow') + '/data_flow_'+ str(module) +'.txt'
        print('\nPath to output_file: ',output_file)
        sys.stdout=open(output_file,"w")  # added by vnay01
        for bk, bv in sorted(binddict.items(), key=lambda x: str(x[0])):
            for bvi in bv:
                print(bvi.tostr())
        sys.stdout.close()                      # added by vnay01
